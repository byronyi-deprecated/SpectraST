...
SpectraSTReplicates::SpectraSTReplicates(vector<SpectraSTLibEntry*>& entries, SpectraSTCreateParams& params, SpectraSTDenoiser* denoiser) :
  m_params(params),
  m_reps(),
  m_numUsed(0),
  m_numTotal(0),
...
void SpectraSTReplicates::processConsensus(SpectraSTLibEntry* consensus, SpectraSTPeakList* consensusPeakList, vector<Replicate*>& usedReps) {
  ...
  if (m_params.maximumNumPeaksKept > 0) {
    double retained = 1.0;
    retained = consensusPeakList->simplify(m_params.maximumNumPeaksKept, 99999.0);
  }
...
//    cerr << dot << endl;
  ...
  if (maxRt >= 0.0) {
    stringstream rtss;
    rtss.precision(1);
    rtss << fixed << showpoint << sumRt / (double)sumNumUsedWithRt << ',';
