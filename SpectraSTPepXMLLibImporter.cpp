...
void SpectraSTPepXMLLibImporter::import() {
  ...
  for (map<string, pair<vector<string>, SpectraSTLibEntry*> >::iterator q = m_queries.begin(); q != m_queries.end(); q++) {
	...
	g_log->log("PEPXML IMPORT", "Too few peaks in spectrum. Skipped query \"" + query + "\"."); 
	m_numSkipped++;
	delete (entry);
	continue;
      }
	...
	peakList->removeITRAQPeaks();
      ...
      // stringstream xreass;
      // xreass.precision(3);
      // xreass << fixed << peakList->calcXrea(true);
      // entry->setOneComment("Xrea", xreass.str());    
      // stringstream snss; 
      // snss.precision(1);
      // snss << fixed << peakList->calcSignalToNoise();
      // entry->setOneComment("SN", snss.str());
      
      // annotate the spectra
      if (!(m_params.skipRawAnnotation)) {
	entry->annotatePeaks();
      }
	...
	g_log->log("PEPXML IMPORT", "Entry did not pass user-defined filter. Skipped query \"" + query + "\".");
	m_numSkipped++;
      }
  ...
  if (m_numQueryWithProb == 0 && m_probCutoff > 0.0000000000001) {
...
void SpectraSTPepXMLLibImporter::readFromFile(string& impFileName) {
  ...
  queryss << "Total of " << m_numQuery << " read (" << m_numQueryWithProb << " with probability; " << m_numQueryWithiProphetProb << " with iProphet probability).";
...
//  delete (searchHitss);
  ...
  if (pep->hasUnknownMod) {
    g_log->error("PEPXML IMPORT", "Peptide ID has unknown modification: \"" + peptide + "\". Skipped query \"" + query + "\".");
    m_numSkipped++;
    delete (pep);
    return (false);
  }
  ...
  if (pep->illegalPeptideStr) {
    g_log->error("PEPXML IMPORT", "Illegal peptide ID string: \"" + peptide + "\". Skipped query \"" + query + "\".");
    m_numSkipped++;
    delete (pep);
    return (false);
  }
  ...
  if (naa < m_params.minimumNumAAToInclude) {
    g_log->log("PEPXML IMPORT", "Peptide ID too short. Skipped query \"" + query + "\"."); 
    m_numSkipped++;
    delete (pep);
    return (false);
  }
...
bool SpectraSTPepXMLLibImporter::loadSpectrum(string query, string path, vector<SpectraSTLibEntry*>& entries, string altPath) {
  ...
  if (firstScanNum != lastScanNum) {
    ...
    if (readOneScan(cramp, scanNum, firstEntry, fullFileName)) {
    ...
    for (int scanNum = firstScanNum + 1; scanNum <= lastScanNum; scanNum++) {
      ...
      if (readOneScan(cramp, scanNum, newEntry, fullFileName, true, firstPrecursorMz - 1.0, firstPrecursorMz + 1.0)) {
	stringstream rawss;
	rawss << baseName << "." << scanNum << "." << scanNum;
	...
	SpectraSTReplicates reps(bracket, m_params);
    ...
    return (readOneScan(cramp, scanNum, entry, fullFileName));
  }
...
bool SpectraSTPepXMLLibImporter::readOneScan(cRamp* cramp, int scanNum, SpectraSTLibEntry* entry, string& fullFileName, bool silent, double minMz, double maxMz) {
...
void SpectraSTPepXMLLibImporter::setDeamidatedNXST(Peptide* pep) { 
...
double SpectraSTPepXMLLibImporter::getProbCutoffFromFDR(double desiredFDR, string& errorsStr, string& minProbsStr, double& actualFDR) {
  ...
  map<double, double, std::greater<double> >::iterator found = cutoffs.upper_bound(desiredFDR + 0.00000001);
