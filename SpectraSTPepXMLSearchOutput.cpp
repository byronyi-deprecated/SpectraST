...
// constructor
SpectraSTPepXMLSearchOutput::SpectraSTPepXMLSearchOutput(string outFullFileName, string inputFileExt, SpectraSTSearchParams& searchParams) :
...
void SpectraSTPepXMLSearchOutput::printHeader() {
  ...
  // Enzyme information is not contained in .mzXML. there's no way SpectraST can know what
  // enzyme is used to digest the sample to begin with. Contrast this with Sequest, for which the
  // enzyme information is entered in sequest.params. 
  //
  // Unfortunately, pepXML REQUIRES a sample_enzyme element.
  // Therefore, SpectraST has to assume it's trypsin and prints out the pepXML element accordingly	
...
void SpectraSTPepXMLSearchOutput::printStartQuery(string query, double precursorMz, int assumedCharge, double retentionTime) {
  ...
  if (m_searchParams.indexRetrievalUseAverage) {
...
void SpectraSTPepXMLSearchOutput::printHit(string query, unsigned int hitRank, SpectraSTLibEntry* entry, SpectraSTSimScores& simScores) {
  ...
  if (!p) {
    ...
    (*m_fout) << noshowpos << "\" num_tol_term=\"" << entry->getNTT();
    (*m_fout) << "\" num_missed_cleavages=\"" << entry->getNMC() << "\">" << endl;
    ...
    (*m_fout).precision(4);
    (*m_fout) << fixed << showpoint << showpos << massDiff;
    (*m_fout) << noshowpos << "\" num_tol_term=\"" << p->NTT();
    (*m_fout) << "\" num_missed_cleavages=\"" << p->NMC() << "\">" << endl;
