...
void SpectraSTTsvLibImporter::import() {
  ...
  countss << "Total of " << m_numImported << " spectra imported, ";
...
void SpectraSTTsvLibImporter::readFromFile(string& impFileName) {
  ...
  while (nextLine(fin, line)) {
    ...
    if (processSearchHit(mzXMLFile, scanNum, id, searchEngine, instrumentType, fn.path, prob, scores)) {
      pc.increment();
    }
...
bool SpectraSTTsvLibImporter::processSearchHit(string& mzXMLFile, int scanNum, string& peptide, string& searchEngine, string& instrumentType, string& path, double& prob, string& scores) {
  ...
  Peptide* pep = new Peptide(peptide, 0);
  
  
  
  if (pep->hasUnknownMod) {
    g_log->error("TSV IMPORT", "Peptide ID has unknown modification: \"" + peptide + "\". Skipped query \"" + query + "\".");
    m_numSkipped++;
    delete (pep);
    return (false);
  }
  ...
  if (pep->illegalPeptideStr) {
    g_log->error("TSV IMPORT", "Illegal peptide ID string: \"" + peptide + "\". Skipped query \"" + query + "\".");
    m_numSkipped++;
    delete (pep);
    return (false);
  }
  ...
  if (naa < m_params.minimumNumAAToInclude) {
    g_log->log("TSV IMPORT", "Peptide ID too short. Skipped query \"" + query + "\"."); 
    m_numSkipped++;
    delete (pep);
    return (false);
  }
...
//  cout << "get peaks" << endl;
  ...
  if (!(fragType.empty()) && entry->getFragType().empty()) {
...
void SpectraSTTsvLibImporter::setDeamidatedNXST(Peptide* pep) { 
