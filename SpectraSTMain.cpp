...
int main(int argc, char* argv[]) {
  ...
  if (!g_quiet) {
    ...
    if (mode == 's' || searchCount > 0) {
      cout << "Total Number of Searches Performed = " << searchCount << "; Run Time per Search = " << timeElapsed / (double)(searchCount) << " seconds." << endl;
    }
    cout.precision(4);
...
void readUserModFile(string& modFileName) {
 ...
 //   string delimiter(" #{}\t\r\n");
	...
	g_log->error("GENERAL", "Illegal user-defined modification token: " + token + " . It must be of the form A[...]. Ignored.");
	continue;
      }        
