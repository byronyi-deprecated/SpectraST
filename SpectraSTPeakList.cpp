...
SpectraSTPeakList::SpectraSTPeakList(vector<SpectraSTPeakList*>& pls, Peptide* pep, unsigned int totalNumRep, double quorum, unsigned int maxNumPeaks, SpectraSTDenoiser* denoiser, bool keepRawIntensities) :
  m_peaks(),
  m_parentMz(0.0),
  m_parentCharge(0),
  m_pep(pep),
  ...
  createConsensusSpectrum(pls, totalNumRep, quorum, maxNumPeaks, denoiser, keepRawIntensities);
...
SpectraSTPeakList& SpectraSTPeakList::operator=(SpectraSTPeakList& other) {
  ...
  this->m_mzAccuracy = other.m_mzAccuracy;
...
void SpectraSTPeakList::insert(double mz, float intensity, string annotation, string info) {
  ...
  if (!annotation.empty()) {
    m_isAnnotated = true;
    if (isBorY(annotation)) {
      m_BYIonCurrent += intensity;
    }
    if (annotation[0] != '?') {
      m_numAssignedPeaks++;
    }
...
void SpectraSTPeakList::setFragType(string& fragType) {
  ...
  if (fragType == "CID-QTOF" || fragType == "HCD") {
...
double SpectraSTPeakList::compare(SpectraSTPeakList* other) {
  ...
  binPeaks(0.0, 0.5, 1.0, 1, 0.5);

  other->binPeaks(0.0, 0.5, 1.0, 1, 0.5);
...
// compare - called from a query peak list object to compare it against a library peak list pointed
// to be 'other'. The method will set the fields in 'simScores' as well as return the simple dot
// product.
double SpectraSTPeakList::compare(SpectraSTPeakList* other, SpectraSTSimScores& simScores, SpectraSTSearchParams& searchParams) {
	
  // pre-processing of both peak lists (note that these functions will not repeat itself if the
  // actions are already taken previously)
  binPeaks(searchParams.peakScalingMzPower, searchParams.peakScalingIntensityPower, searchParams.peakScalingUnassignedPeaks,
           searchParams.peakBinningNumBinsPerMzUnit, searchParams.peakBinningFractionToNeighbor);
 
  other->binPeaks(searchParams.peakScalingMzPower, searchParams.peakScalingIntensityPower, searchParams.peakScalingUnassignedPeaks,
                  searchParams.peakBinningNumBinsPerMzUnit, searchParams.peakBinningFractionToNeighbor);
  
  // sumDotSquares is used to calculate the dot bias. In brief, sumDotSquares = summation of the 
  // squares of each term that contributes to the dot product, i.e.
  // Dot = sum_i (a_i * b_i) and sumDotSquares = sum_i [(a_i * b_i)^2] where i is the bin number
  simScores.dot = calcDotAndDotBias(other, simScores.dotBias);

  return (simScores.dot);
}
...
void SpectraSTPeakList::scalePeaks(double mzPower, double intensityPower, double unassignedFactor, bool rescale) {
  ...
  for (i = m_peaks.begin(); i != m_peaks.end(); i++) {
    i->intensity = scale(*i, mzPower, intensityPower, unassignedFactor);
  }
...
// scale - scale peaks.
float SpectraSTPeakList::scale(Peak& p, double mzPower, double intensityPower, double unassignedFactor, bool removePrecursor) {
...
// binPeaks - put peaks in bins, scaling them first. 
void SpectraSTPeakList::binPeaks(double mzPower, double intensityPower, double unassignedFactor,
 ...
 if (m_binIndex) {
    ...
    for (vector<Peak>::iterator i = m_peaks.begin(); i != m_peaks.end(); i++) {
      ...
      float inten = scale(*i, mzPower, intensityPower, unassignedFactor, true);	
      ...
      if (shift < 0) {
        ...
        delete (m_binIndex);
        m_binIndex = NULL;
        binPeaks(mzPower, intensityPower, unassignedFactor, numBinsPerMzUnit, fractionToNeighbor);
        return;
      }
...
Peak* SpectraSTPeakList::annotateIon(FragmentIon* fi, bool fixMz) {
  ...
  bool heavy = ((mz * (double)charge) > 1400.0); 
  // true if ion so heavy that the monoisotopic peak is no longer the biggest in the envelope
  // in those cases, make the tolerance on the +ve side bigger so that it can catch the biggest peak -- which
  // sometimes is at the centroided m/z - sometimes 1 Da or so higher than the monoisotopic. 
  // it will probably favor this biggest peak at a farther away m/z from the monoisotopic and annotate this big peak instead
  // (Yeah, the annotateIsotopicIon part will be somewhat inappropriate which is less of a concern than missing big peaks!)
 ...
 // find all peaks within the tolerable range
  map<double, pair<Peak*, double> >::iterator low = m_peakMap->upper_bound(mz - m_mzAccuracy);
  map<double, pair<Peak*, double> >::iterator high = m_peakMap->lower_bound(mz + m_mzAccuracy + extraTolerance);
  ...
  if (fixMz && (bestPeak->annotation.empty() || bestPeak->annotation[0] == '[')) {
    ...
    bestPeak->mz = mz;
    bestPeak->annotation = fi->m_ion + "/0.00";
...
map<double, pair<Peak*, double> >::iterator SpectraSTPeakList::findBestPeakToAssign(double mzTh, map<double, pair<Peak*, double> >::iterator low, map<double, pair<Peak*, double> >::iterator high) {
  ...
  for (i = low; i != high; i++) {
    double delta = fabs(i->second.first->mz - mzTh) / m_mzAccuracy;
...
void SpectraSTPeakList::annotateIsotopicIons(Peak* assignedMonoisotopic, FragmentIon* fi, bool fixMz) {
  ...
  if (m_mzAccuracy < 0.3) isotopicPeakMzTolerance = m_mzAccuracy < 0.05 ? 0.05 : m_mzAccuracy;
  ...
  if (fixMz && (bestPeak->annotation.empty() || bestPeak->annotation[0] == '[')) {
    ...
    bestPeak->annotation = fi->m_ion + "i/0.00";
...
void SpectraSTPeakList::createConsensusSpectrum(vector<SpectraSTPeakList*>& pls, unsigned int totalNumRep, double quorum, unsigned int maxNumPeaks, SpectraSTDenoiser* denoiser, bool keepRawIntensities) {
	
  unsigned int minNumRepWithPeak = (unsigned int)(totalNumRep * quorum - 0.00001) + 1;
  if (minNumRepWithPeak < 1) minNumRepWithPeak = 1;
	
  double alignMzAccuracy = m_mzAccuracy;
  if (alignMzAccuracy < 0.05) alignMzAccuracy = 0.05;
  ...
  if (denoiser) {
    ...
    for (vector<SpectraSTPeakList*>::iterator pl = pls.begin(); pl != pls.end(); pl++) {
      ...
      denoiser->addTrainingSpectrum(*pl);
...
void SpectraSTPeakList::rankByIntensity(bool redo) {
...
double SpectraSTPeakList::calcXCorr() {
  ...
  binPeaks(0.0, 1.0, 1.0, 1, 0.0, true); // force rebinning
...
void SpectraSTPeakList::removeITRAQPeaks() {
...
double SpectraSTPeakList::simplify(unsigned int maxNumPeaks, double maxDynamicRange, bool deisotope, bool excludeParent) {
  
  if ((unsigned int)(m_peaks.size()) <= maxNumPeaks 
       && m_origMaxIntensity <= maxDynamicRange 
       && !deisotope && !excludeParent) return (1.0);
  ...
  if (!deisotope) {
    for (unsigned int r = 0; r < maxNumPeaks && r < numPeaks; r++) {
      ...
      if (excludeParent && isNearPrecursor(p.mz)) {
        maxNumPeaks++;
      } else {
  ...
  for (vector<Peak>::iterator i = m_peaks.begin(); i != m_peaks.end(); i++) {
    if (excludeParent && isNearPrecursor(i->mz)) {
      continue;
    }
    string::size_type pos = 0;
...
void SpectraSTPeakList::getNthLargestPeak(unsigned int n, Peak& p) {
  ...
  if (n < 1 || n > (unsigned int)(m_peaks.size())) {
    p.mz = 0.0;
    p.intensity = 0.0;
    p.annotation = "";
    p.info = "";
...
/*
// reduce - previous version

// reduce - reduces the spectra to at most maxNumPeaks with
// some rules. work in progress...
double SpectraSTPeakList::reduce(unsigned int maxNumPeaks, unsigned int numRepsUsed) {
  
  // just take the top maxNumPeaks for now
  return (simplify(maxNumPeaks, 1000000, false, false));
...
double SpectraSTPeakList::reduce(unsigned int maxNumPeaks, double minMz, double maxMz, unsigned int numRepsUsed) {
  ...
  for (unsigned int r = 0; r < m_intensityRanked->size(); r++, rankWithoutBadPeaks++) {
    ...
    // top 20 peaks or at least 20% of base intensity
    if (rankWithoutBadPeaks < 20 || (p->intensity / maxIntensity > 0.2)) {
